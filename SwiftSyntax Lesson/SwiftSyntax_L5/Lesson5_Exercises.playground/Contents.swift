...
func combineLastCharacters (array: [String]) -> String {
    var newString = ""
    for words in array {
        newString.append(lastLetterOf(word: words))
    }
    return newString
...
let dirtyWordsArray = ["phooey", "darn", "drat", "blurgh", "jupiters", "argh", "fudge","baila"]
...
class MovieArchive {
    ...
    func filterByDirector (MovieList: Dictionary<String, String>, Director: String) -> [String] {
        
        var movieCollection = [String]()
        for (title, name) in MovieList {
            
            if name == Director {
                
                movieCollection.append(title)
...
var directorList = MovieArchive()
directorList.filterByDirector(movies, Director: "Christopher Nolan")
