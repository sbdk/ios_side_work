...
class LoginViewController: UIViewController {
    ...
    func loginWithToken(requestToken: String) {
        ...
        let methodParamethers: [String : String!] = [
            "api_key": appDelegate.apiKey,
        ...
        /* 2. Build the URL */
        let urlString = appDelegate.baseURLSecureString + "authentication/token/validate_with_login" + appDelegate.escapedParameters(methodParamethers)
        let url = NSURL(string: urlString)!
        ...
        let task = session.dataTaskWithRequest(request){(data, response, error) in
            ...
            guard (error == nil) else {
                dispatch_async(dispatch_get_main_queue()) {
                    self.debugTextLabel.text = "Login Failed (wrong username or password)."
                }
    ...
    func getSessionID(requestToken: String) {
        ...
        let task = session.dataTaskWithRequest(request) {(data, response, error) in
            ...
            guard let session_ID = parsedResult["session_id"] as? String else{
                ...
                print("Cannot find key 'seesionID' in \(parsedResult)")
                return
            }
    ...
    func getUserID(session_id : String) {
        ...
        let task = session.dataTaskWithRequest(request) {(data, response, error) in
            ...
            guard let userID = parsedResult["id"] as? Int else {
                dispatch_async(dispatch_get_main_queue()) {
                    self.debugTextLabel.text = "Login Failed (Not a valid User account)."
                }
                print("Cannot find a valid userID in \(parsedResult)")
                return
            }
            
            self.appDelegate.userID = userID
            print("get SessionID: \(userID)")
